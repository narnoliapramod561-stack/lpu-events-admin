...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Domain 2 lock: publish flows through the publish_event RPC only.
        if (data.status === 'published') {
            return NextResponse.json({
                error: 'INVALID_TRANSITION',
                message: 'Cannot publish via update. Use POST /api/organizer/events/[id]/publish (publish_event RPC).'
            }, { status: 400 });
        }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Ownership bypass applies ONLY to admins. Organizer behaviour is
        // unchanged: an organizer may delete ONLY their own event.
        if (!isOwner && !isAdmin) {
            return NextResponse.json({ error: 'FORBIDDEN', message: 'Only the event organizer can delete this event' }, { status: 403 });
        }

        // Organizers keep the existing draft-only deletion rule. Since events
        // in this platform are published immediately (no draft workflow), the
        // draft-only rule would make deletion unusable for the Super Admin; an
        // admin may therefore delete any non-archived event.
        ...
        // Perform the soft delete with the service-role (privileged) client.
        // Authorization (owner + draft) has already been enforced above, so this
        // write does not depend on the cookie-based RLS context. Using the cookie
        // client here could issue the UPDATE as the `anon` role on serverless
        // runtimes (auth.uid() = NULL), causing the events UPDATE policy's
        // WITH CHECK (organizer_id = auth.uid()) to fail with:
